...
def load_api_key(apikey_path: str | None = None) -> str:
    if apikey_path:
        path = Path(apikey_path)
        if path.exists():
            return path.read_text(encoding="utf-8").splitlines()[0].strip()
        raise FileNotFoundError(f"API key file not found: {apikey_path}")
    if os.getenv("OPENAI_API_KEY"):
        return os.environ["OPENAI_API_KEY"].strip()
    raise FileNotFoundError("Provide --apikey-txt or set OPENAI_API_KEY environment variable")
...
def answer_questions(input_rows: list[dict], client: OpenAI, model: str) -> list[dict]:
...
def main() -> int:
    ...
    parser.add_argument("--corpus-dir", required=True, help="Root of the .rst corpus (the released sourcedocs/ layout).")
    parser.add_argument("--apikey-txt", required=True, help="Path to the text file of the TritonAI gateway API key.")
    parser.add_argument("--generation-model", required=True, help="Gateway model id (e.g. claude-sonnet-4-6, api-gpt-oss-120b) for generating answers to queries.")
    ...
    output_rows = answer_questions(input_rows, client, args.generation_model)
